...
class SerialOSC:
    def __init__(self):
        ...
        def default_handler(address, *args):
            logger.warning(
                "SerialOSC: No handler for message: %s %s" % (address, args))
        ...
        self.server = ThreadingOSCUDPServer(
            (SERIALOSC_HOST, SERIALOSC_CLIENT_PORT), dispatcher)
        self.thread = threading.Thread(
            target=self.server.serve_forever, daemon=True)
        self.thread.start()
        self.client = SimpleUDPClient(SERIALOSC_HOST, SERIALOSC_SERVER_PORT)
        self.client.send_message(
            "/serialosc/list", [SERIALOSC_HOST, SERIALOSC_CLIENT_PORT])
        self.available_devices: list[DeviceSpec] = []

    def _serialosc_register(self):
        self.client.send_message(
            "/serialosc/notify", [SERIALOSC_HOST, SERIALOSC_CLIENT_PORT])

    def handle_device_listed(self, address, device_id, device_model, port):
        logger.warning("Found serial OSC device: %s (model %s, port %d)" % (
            device_id, device_model, port))
    ...
    def handle_device_added(self, address, device_id, device_model, port):
        logger.warning("Added serial OSC device: %s (model %s, port %d)" % (
            device_id, device_model, port))
        device = DeviceSpec(device_id, device_model, port)
    ...
    def handle_device_removed(self, address, device_id, device_model, port):
        logger.warning("Removed serial OSC device: %s (model %s, port %d)" % (
            device_id, device_model, port))
        device = DeviceSpec(device_id, device_model, port)
        self.available_devices.remove(device)
        self._serialosc_register()
